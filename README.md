...
Codex CLI は `AGENTS.md` を**カレントディレクトリ（および親ディレクトリ）から自動で読み込みます**。Claude Code の skills フォルダのような「グローバル配置フォルダ」はありません。

#### 方法 A：pptx-reviewer フォルダ内から起動する（テスト・動作確認向け）

最もシンプルな方法です。Codex CLI をこのリポジトリのフォルダ内から起動すると、`AGENTS.md` が自動で読み込まれます。
...
この方法ではスクリプトのパスを `./scripts/` と書けばそのまま動きます。

#### 方法 B：どのフォルダからでも使えるようにする（グローバル運用向け）

`AGENTS.md` をホームディレクトリに置くことで、どこから `codex` を起動しても読み込まれます。

**手順：**

1. ホームディレクトリに `AGENTS.md` をコピーする

```cmd
copy "%USERPROFILE%\pptx-review-text\AGENTS.md" "%USERPROFILE%\AGENTS.md"
```

2. コピーした `%USERPROFILE%\AGENTS.md` をメモ帳などで開き、`SKILL_DIR` を実際のパスに置き換える

```
変更前： SKILL_DIR/scripts/extract_pptx.py
変更後： C:\Users\（ユーザー名）\pptx-review-text\scripts\extract_pptx.py
```
...
```
C:\Users\（ユーザー名）\
├── AGENTS.md                         ← ここに置くとどこからでも読み込まれる
└── pptx-review-text\                 ← リポジトリ本体（スクリプト置き場）
    ├── AGENTS.md                     ← 元ファイル（テンプレート）
    ├── SKILL.md
    ├── scripts\
    └── references\
```

> **注意**：すでにホームディレクトリに `AGENTS.md` がある場合は、上書きせずファイルの末尾に内容を**追記**してください。
...
**テストする場合（pptx-reviewer フォルダ内から起動）：**

追加の配置作業は不要です。クローンしたフォルダに `AGENTS.md` がすでに入っています。
...
**どこからでも使いたい場合（グローバル配置）：**

```cmd
:: AGENTS.md をホームディレクトリにコピー
copy "%USERPROFILE%\pptx-review-text\AGENTS.md" "%USERPROFILE%\AGENTS.md"
```

コピー後、`%USERPROFILE%\AGENTS.md` をメモ帳で開き、`SKILL_DIR` を実際のパスに置き換えます。
